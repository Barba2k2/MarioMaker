...
class Editor:
        def __init__(self, land_tiles, switch):
            ...
            # clouds
            self.current_clouds = []
            self.cloud_surf = import_folder('G:/Meu Drive/Pygame/MarioMaker/graphics/clouds')
        ...
        def imports(self):
            ...
            self.sky_handle_surf = load('G:/Meu Drive/Pygame/MarioMaker/graphics/cursors/handle.png').convert_alpha()
            ...
            for tile_pos, tile in self.canvas_data.items():
                ...
                if tile.objects:
                    for obj, offset in tile.objects:
                        if obj in [key for key, value in EDITOR_DATA.items() if value['style'] == 'palm_bg']: # bg plams
        ...
        def preview(self):
            ...
            if not self.menu.rect.collidepoint(mouse_pos()):
                if selected_object:
                    ...
                    type_dict = {key: value for key, value in EDITOR_DATA.items()}
                    surf = self.preview_surfs[self.selection_index].copy()
                    surf.set_alpha(200)
                    
                    # tile
                    if type_dict[self.selection_index] == 'tile':
                        current_cell = self.get_curent_cell()
