...
class Menu:
    ...
    def create_buttons(self):
        ...
        # button areas
        genereic_button_rect = pygame.Rect(self.rect.topleft,(self.rect.width / 2, self.rect.height / 2))
        button_margin = 5
        self.tile_button_rect = genereic_button_rect.copy().inflate(-button_margin, -button_margin)
        self.coin_button_rect = genereic_button_rect.move(self.rect.height / 2, 0).inflate(-button_margin, -button_margin)
        self.enemy_button_rect = genereic_button_rect.move(self.rect.height / 2, self.rect.width / 2).inflate(-button_margin, -button_margin)
        self.palm_button_rect = genereic_button_rect.move(0, self.rect.width / 2).inflate(-button_margin, -button_margin)
    ...
    def click(self, mouse_pos, mouse_button):
        for sprite in self.buttons:
            if sprite.rect.collidepoint(mouse_pos):
                ...
                if mouse_button[2]: # richt click
                    sprite.switch()
                return sprite.get_id()
    
    def highlight_indicator(self, index):
        if EDITOR_DATA[index]['menu'] == 'terrain':
            pygame.draw.rect(self.display_surface, BUTTON_LINE_COLOR, self.tile_button_rect.inflate(), 5, 4)
        if EDITOR_DATA[index]['menu'] == 'coin':
            pygame.draw.rect(self.display_surface, BUTTON_LINE_COLOR, self.coin_button_rect.inflate(), 5, 4)
        if EDITOR_DATA[index]['menu'] == 'enemy':
            pygame.draw.rect(self.display_surface, BUTTON_LINE_COLOR, self.enemy_button_rect.inflate(), 5, 4)
        if EDITOR_DATA[index]['menu'] in ('palm bg', 'palm_fg'):
            pygame.draw.rect(self.display_surface, BUTTON_LINE_COLOR, self.palm_button_rect.inflate(), 5, 4)
...
class Button(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        rect = surf.get_rect(center = (self.rect.width / 2, self.rect.width / 2))
