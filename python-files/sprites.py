...
class Cloud(Generic):
    def __inti__(self, pos, surf, group, left_limit):
    ...
    def update(self, dt):
        ...
        if self.rect.x <= self.left.limit:
            self.kill()
...
class Tooth(Generic):
    def __init__(self, assets, pos, group, collision_sprites):
        ...
        # detroy tooth at the beginning if he isnt on a floor
    ...
    def move(self, dt):
        ...
        left_block = self.midleft + vector (-1, 0)
...
class Player(Generic):
    ...
    def damage(self):
        if not self.invul_timer.activate():
